...
const yamlContent = readFileSync(join(import.meta.dir, '..', 'data', 'archetypes', 'standard.yaml'), 'utf-8');
...
for (const [archetypeName, playerCount] of sortedArchetypes.slice(0, 3)) {
	...
	const cardStats: { card: string; dominantFreq: number; dominantCount: number; uniqueCounts: number; isBasicLand: boolean }[] = [];
	...
	for (const threshold of buckets) {
		const count = cardStats.filter(c => c.dominantFreq > prev && c.dominantFreq <= threshold).length;
		const basics = cardStats.filter(c => c.dominantFreq > prev && c.dominantFreq <= threshold && c.isBasicLand).length;
		console.log(`  ${(prev * 100).toFixed(0)}%-${(threshold * 100).toFixed(0)}%: ${count} cards (${basics} basic lands)`);
		prev = threshold;
	}

	const total = cardStats.length;
	const totalBasics = cardStats.filter(c => c.isBasicLand).length;
	...
	for (const t of [0.8, 0.85, 0.9, 0.95]) {
		...
		console.log(`  >=${(t * 100).toFixed(0)}%: ${filtered.length}/${total} filtered (${filteredBasics} basics), ${remaining} remaining for testing`);
	}
	...
	for (const c of boundary.slice(0, 15)) {
		console.log(`  ${c.card}: ${(c.dominantFreq * 100).toFixed(1)}% at ${c.dominantCount} copies, ${c.uniqueCounts} unique counts${c.isBasicLand ? ' [BASIC]' : ''}`);
	}
	...
	for (const c of variable.slice(0, 10)) {
		console.log(`  ${c.card}: ${(c.dominantFreq * 100).toFixed(1)}% at ${c.dominantCount} copies, ${c.uniqueCounts} unique counts`);
	}
}
