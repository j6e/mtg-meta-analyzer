...
async function main() {
	...
	try {
		...
		if (standings.length > 0) {
			...
			if (first.Decklists.length > 0) {
				...
				console.log(
					`   Decklist: ${first.Decklists[0].DecklistName} (${first.Decklists[0].DecklistId})`,
				);
			}
		}
	} catch (e) {
	...
	if (decklistId) {
		...
		try {
			...
			console.log(
				`   Mainboard: ${mainboard.length} unique cards, Sideboard: ${sideboard.length} unique cards`,
			);
		} catch (e) {
			logError(e);
		}
	...
	try {
		...
		if (ended.length > 0) {
			console.log(
				`   Latest: "${ended[0].Name}" (${ended[0].FormatDescription}, ${ended[0].Decklists} decklists)`,
			);
		}
	} catch (e) {
		logError(e);
