...
import type {
	ArchetypeStats,
	MatchupMatrix as MatrixType,
} from "../../src/lib/types/metagame";
...
describe("MatchupMatrix component", () => {
	...
	it("displays archetype names in headers", () => {
		...
		const rowNames = [...rowHeaders].map((th) =>
			th.querySelector(".archetype-link")?.textContent?.trim(),
		);
