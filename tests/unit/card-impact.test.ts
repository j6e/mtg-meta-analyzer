...
function makePlayer(name: string, decklistIds: string[], rank = 1): PlayerInfo {
	return {
		name, username: name.toLowerCase(), rank,
		points: 0, matchRecord: '0-0-0',
		decklistIds, reportedArchetypes: [],
	};
}

function makeDeck(playerId: string, mainboard: [string, number][], sideboard: [string, number][] = []): DecklistInfo {
	return {
		playerId,
...
function makeMatch(p1: string, p2: string, winnerId: string | null): MatchResult {
	return { player1Id: p1, player2Id: p2, result: winnerId ? '2-1-0' : '1-1-0', winnerId };
}
...
function makeTournament(overrides: {
	...
	return {
		meta: {
			id: 1, name: 'Test', date: '2026-01-01', formats: ['Standard'],
			url: 'https://melee.gg/Tournament/View/1', fetchedAt: '2026-01-01T00:00:00Z',
			playerCount: Object.keys(overrides.players).length, roundCount: 1,
		},
		players: overrides.players,
		decklists: overrides.decklists,
		rounds: {
			r1: { name: 'Round 1', number: 1, isPlayoff: false, matches: overrides.matches },
		},
	};
}
...
const tournament = makeTournament({
	...
	decklists: {
		da1: makeDeck('a1', [['Lightning Bolt', 4], ['Shock', 0], ['Mountain', 20]]),
		da2: makeDeck('a2', [['Lightning Bolt', 4], ['Shock', 0], ['Mountain', 20]]),
		da3: makeDeck('a3', [['Lightning Bolt', 2], ['Shock', 2], ['Mountain', 20]]),
		da4: makeDeck('a4', [['Lightning Bolt', 2], ['Shock', 2], ['Mountain', 20]]),
		da5: makeDeck('a5', [['Lightning Bolt', 0], ['Shock', 4], ['Mountain', 20]]),
		da6: makeDeck('a6', [['Lightning Bolt', 0], ['Shock', 4], ['Mountain', 20]]),
		dc1: makeDeck('c1', [['Island', 20], ['Counterspell', 4]]),
		dc2: makeDeck('c2', [['Island', 20], ['Counterspell', 4]]),
		dc3: makeDeck('c3', [['Island', 20], ['Counterspell', 4]]),
	},
	matches: [
		// Bolt users win more
		makeMatch('a1', 'c1', 'a1'),
...
const archetypes = new Map([
	['a1', 'Aggro'], ['a2', 'Aggro'], ['a3', 'Aggro'],
	['a4', 'Aggro'], ['a5', 'Aggro'], ['a6', 'Aggro'],
...
describe('extractTrainingData', () => {
	...
	it('win/loss labels match winnerId', () => {
		...
		const a1obs = obs.find((o) => o.cardCounts.get('Lightning Bolt') === 4 && o.outcome === 1);
	...
	it('excludes mirrors', () => {
		...
		const mirrorArch = new Map([['a1', 'Aggro'], ['a2', 'Aggro']]);
...
describe('selectFlexFeatures', () => {
	it('card with identical counts across all decks is excluded', () => {
		const obs: TrainingObservation[] = [
			{ cardCounts: new Map([['Same', 4], ['Varies', 1]]), outcome: 1 },
			{ cardCounts: new Map([['Same', 4], ['Varies', 3]]), outcome: 0 },
			{ cardCounts: new Map([['Same', 4], ['Varies', 2]]), outcome: 1 },
		];
	...
	it('basic lands are excluded', () => {
		const obs: TrainingObservation[] = [
			{ cardCounts: new Map([['Mountain', 20], ['Bolt', 4]]), outcome: 1 },
			{ cardCounts: new Map([['Mountain', 18], ['Bolt', 2]]), outcome: 0 },
		];
		const features = selectFlexFeatures(obs);
		const names = features.map((f) => f.cardName);
	...
	it('auto-include cards (>90% same count) are excluded', () => {
		...
		for (let i = 0; i < 20; i++) {
			...
			obs.push({
				cardCounts: new Map([['AutoInclude', autoCount], ['Flex', i % 4]]),
				outcome: i % 2,
			});
		}
		const features = selectFlexFeatures(obs);
...
describe('analyzeCardImpact', () => {
	...
	it('no flex cards returns error', () => {
		...
		const uniformTournament = makeTournament({
			...
			matches: Array.from({ length: 30 }, (_, i) =>
				makeMatch(i % 2 === 0 ? 'a1' : 'a2', 'c1', i % 3 === 0 ? 'c1' : (i % 2 === 0 ? 'a1' : 'a2')),
			),
		});
		const uniformArch = new Map([['a1', 'X'], ['a2', 'X'], ['c1', 'Y']]);
	...
	it('card correlating with wins has positive coefficient', () => {
		...
		if ('regression' in result) {
			...
			const boltCoef = result.regression.coefficients.find((c) => c.name === 'Lightning Bolt');
			if (boltCoef) {
