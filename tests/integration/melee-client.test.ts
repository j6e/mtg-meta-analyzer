...
import { MeleeApiError, MeleeClient } from "../../scripts/lib/melee-client";
import type {
	DataTablesResponse,
	MeleeMatchRow,
	MeleeStandingRow,
...
function standingsPage(
	data: Partial<MeleeStandingRow>[],
	total: number,
): DataTablesResponse<MeleeStandingRow> {
	return {
		draw: 1,
		recordsTotal: total,
		...
		data: data.map((d) => ({
			...
			Team: {
				Players: [{ ID: 1, DisplayName: "Test Player", Username: "test", TeamId: 1 }],
			},
			Decklists: [],
			...d,
		})),
...
describe("MeleeClient", () => {
	...
	describe("fetchAllStandings", () => {
		...
		it("paginates through multiple pages", async () => {
			...
			const secondBody = (fetchSpy.mock.calls[1] as [string, RequestInit])[1]
				.body as string;
	...
	describe("fetchDecklistDetails", () => {
		it("sends GET to JSON endpoint with GUID id", async () => {
			...
			const result = await client.fetchDecklistDetails(
				"95bc7d4d-ff64-4f5a-9219-76093c05d5ff",
			);

			const [url] = fetchSpy.mock.calls[0] as [string, RequestInit];
			expect(url).toBe(
				"https://melee.gg/Decklist/GetDecklistDetails?id=95bc7d4d-ff64-4f5a-9219-76093c05d5ff",
			);
		...
		it("throws on API error response (200 with error body)", async () => {
			...
			await expect(client.fetchDecklistDetails("nonexistent")).rejects.toThrow(
				MeleeApiError,
			);
		});
	});
